...
serve(async (req) => {
  ...
  try {
    console.log('Enhanced Gemini interview function called')
    ...
    switch (type) {
      ...
      case 'interview-questions':
        ...
        break
      case 'evaluation':
        response = await evaluateAnswer(question, answer, apiKey)
        break
      case 'feedback':
    ...
    console.error('Error in enhanced gemini-interview function:', error)
    return new Response(
...
// Generate interview questions (legacy support)
async function generateInterviewQuestions(prompt: string, apiKey: string) {
  console.log('Generating interview questions for prompt:', prompt)
  
  const questionPrompt = `Generate 5-10 professional interview questions for the role: ${prompt}
...
- Experience-based questions`
  ...
  if (!content) {
    throw new Error('No content generated for interview questions')
  }
  ...
  try {
    ...
    console.error('JSON parse error for interview questions:', parseError)
    
    // Fallback questions
    const fallbackQuestions = [
      `Tell me about your experience with ${prompt}`,
      `What challenges have you faced in ${prompt} and how did you overcome them?`,
      `How do you stay current with developments in ${prompt}?`,
      `Describe a project where you used ${prompt} skills`,
      `What would you do if you encountered a problem you couldn't solve in ${prompt}?`
    ]
...
Provide feedback in JSON format:
{
  ...
  try {
    ...
    const fallbackEvaluation = {
      score: 6.0,
      feedback: 'Answer provided but could be improved with more specific examples',
      strengths: ['Attempted to answer the question'],
      improvements: ['Add more specific examples', 'Use structured approach like STAR method']
    }
...
async function getAnswerFeedback(question: string, answer: string, apiKey: string) {
  console.log('Getting answer feedback')
  
  const feedbackPrompt = `Provide constructive feedback for this interview answer:
...
Return JSON with feedback:
{
  ...
  try {
    ...
    console.error('JSON parse error for feedback:', parseError)
    return new Response(JSON.stringify(null), {
      headers: { 'Content-Type': 'application/json' }
    })
  }
