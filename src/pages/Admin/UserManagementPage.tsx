  ...
  const handleRemovePro = async (userId: string) => {
    try {
      ...
      if (!existingSubscription) {
        ...
        toast({ title: 'No Pro Plan Found', description: 'This user does not have an active Pro plan to remove.', variant: 'destructive' });
        return;
      }

      console.log('Found existing subscription, attempting deletion...');
      ...
      if (deleteError) {
        console.error('Error removing pro plan:', deleteError);
        toast({ 
          title: 'Error', 
          description: `Failed to remove Pro plan: ${deleteError.message}`, 
          variant: 'destructive' 
        });
        return;
      }

      console.log('Pro plan successfully removed for user:', userId);
      toast({ 
        title: 'Pro Plan Removed', 
        description: 'User\'s Pro plan has been successfully removed.' 
      });
      
      // Refresh the user list
      fetchUsers();
      console.log('=== REMOVE PRO DEBUG END ===');
    } catch (e) {
      console.error('Error in handleRemovePro:', e);
      toast({ 
        title: 'Error', 
